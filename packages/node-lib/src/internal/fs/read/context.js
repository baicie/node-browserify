...
const { ArrayPrototypePush, MathMin, ReflectApply } = primordials

const {
  constants: { kReadFileBufferLength, kReadFileUnknownBufferLength },
...
function readFileAfterClose(err) {
  ...
  try {
    if (context.size === 0) buffer = Buffer.concat(context.buffers, context.pos)
