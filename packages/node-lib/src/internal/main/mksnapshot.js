...
const {
  BuiltinModule: { normalizeRequirableId },
} = require('internal/bootstrap/realm')
const { compileSerializeMain, anonymousMainPath } =
...
function main() {
  ...
  addSerializeCallback(() => {
    stackTraceLimitDesc = ObjectGetOwnPropertyDescriptor(
      Error,
      'stackTraceLimit',
    )
    ...
    if (stackTraceLimitDesc !== undefined) {
      ...
      process._rawDebug(
        'Deleting Error.stackTraceLimit from the snapshot. ' +
