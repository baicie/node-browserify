...
function isNativeUrl(url) {
  ...
  return (
    StringPrototypeStartsWith(url, 'node:internal/') ||
...
function markSourceColumn(sourceText, position, useColors) {
  ...
  if (useColors) {
    tail = SideEffectFreeRegExpPrototypeSymbolReplace(
      /(.+?)([^\w]|$)/,
      tail,
      '\u001b[32m$1\u001b[39m$2',
    )
...
class ScopeSnapshot {
  ...
  [customInspectSymbol](depth, opts) {
    ...
    return SideEffectFreeRegExpPrototypeSymbolReplace(
      /^Map /,
...
function createRepl(inspector) {
  ...
  async function formatWatchers(verbose = false) {
    ...
    const values = await SafePromiseAllReturnArrayLike(
      watchedExpressions,
      inspectValue,
    )
  ...
  Debugger.on('paused', ({ callFrames, reason /* , hitBreakpoints */ }) => {
    if (
      ...
      debuglog(
        'Paused on start, but NODE_INSPECT_RESUME_ON_START' +
    ...
    inspector.suspendReplWhile(() =>
      PromisePrototypeThen(
        SafePromiseAllReturnArrayLike([
          formatWatchers(true),
          selectedFrame.list(contextLineNumber),
        ]),
