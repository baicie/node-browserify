...
function truncateStart(string, width) {
  return string.length > width
    ? `${kHorizontalEllipsis}${StringPrototypeSlice(string, string.length - width + 1)}`
    : string
}

function truncateEnd(string, width) {
  return string.length > width
    ? `${StringPrototypeSlice(string, 0, width - 1)}${kHorizontalEllipsis}`
    : string
}
...
function getUncoveredLines(lines) {
  return ArrayPrototypeFlatMap(lines, (line) =>
...
function getCoverageReport(pad, summary, symbol, color, table) {
  ...
  if (table) {
    ...
    filePadLength =
      ...
      ArrayPrototypeReduce(
        summary.files,
        (acc, file) =>
          MathMax(acc, relative(summary.workingDirectory, file.path).length),
        0,
      )
    filePadLength = MathMax(filePadLength, 'file'.length)
    const fileWidth = filePadLength + 2

    columnPadLengths = ArrayPrototypeMap(kColumns, (column) =>
      table ? MathMax(column.length, 6) : 0,
    )
    const columnsWidth = ArrayPrototypeReduce(
      columnPadLengths,
    ...
    uncoveredLinesPadLength = MathMax(
      uncoveredLinesPadLength,
  ...
  report +=
    ...
    `${ArrayPrototypeJoin(
      ArrayPrototypeMap(kColumns, (column, i) =>
  ...
  for (let i = 0; i < summary.files.length; ++i) {
    ...
    report +=
      ...
      `${ArrayPrototypeJoin(
        ArrayPrototypeMap(coverages, (coverage, j) =>
          getCell(
            NumberPrototypeToFixed(coverage, 2),
            columnPadLengths[j],
            StringPrototypePadStart,
            false,
            coverage,
  ...
  report +=
    ...
    `${ArrayPrototypeJoin(
      ArrayPrototypeMap(kColumnsKeys, (columnKey, j) =>
        getCell(
          NumberPrototypeToFixed(summary.totals[columnKey], 2),
          columnPadLengths[j],
          StringPrototypePadStart,
          false,
