...
function createTestFileList() {
  ...
  const patterns = hasUserSuppliedPattern
  ...
  if (
    ...
    results.length === 0 &&
    ArrayPrototypeEvery(glob.matchers, (m) => !m.hasMagic())
  ) {
...
function getRunArgs(path, { forceExit, inspectPort, testNamePatterns, only }) {
  ...
  if (testNamePatterns != null) {
    ArrayPrototypeForEach(testNamePatterns, (pattern) =>
      ArrayPrototypePush(argv, `--test-name-pattern=${pattern}`),
    )
  }
  if (only === true) {
...
class FileTest extends Test {
  ...
  #drainReportBuffer() {
    if (this.#reportBuffer.length > 0) {
      ArrayPrototypeForEach(this.#reportBuffer, (ast) =>
...
function run(options = kEmptyObject) {
  ...
  if (shard != null) {
    ...
    if (shard.index <= 0 || shard.total < shard.index) {
      throw new ERR_OUT_OF_RANGE(
        'options.shard.index',
        `>= 1 && <= ${shard.total} ("options.shard.total")`,
        shard.index,
      )
    }
  ...
  PromisePrototypeThen(
    PromisePrototypeThen(PromiseResolve(setup?.(root.reporter)), runFiles),
    postRun,
  )
