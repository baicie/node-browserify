...
const {
  privateSymbols: { entry_point_module_private_symbol },
    ...
    try {
      ...
      if (
        ...
        const packageConfig =
          ...
          RegExpPrototypeExec(/\.js(\?[^#]*)?(#.*)?$/, this.module.url) !==
            null &&
...
class ModuleJobSync extends ModuleJobBase {
  constructor(loader, url, importAttributes, moduleWrap, isMain, inspectBrk) {
    ...
    for (let i = 0; i < moduleRequests.length; ++i) {
      ...
      const wrap = this.loader.getModuleWrapForRequire(
        specifier,
        url,
        attributes,
      )
      ...
      this.module.cacheResolvedWrapsSync(
        specifier,
        PromiseResolve(wrap),
        isLast,
      )
