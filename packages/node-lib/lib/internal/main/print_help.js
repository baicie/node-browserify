...
const typeLookup = []
for (const key of ObjectKeys(types)) typeLookup[types[key]] = key
  ...
  ArrayPrototypeConcat(
    [
      [
        ...
        {
          helpText:
            "when set to 'true', 1, 2, 3, or an empty " +
      ...
      [
        ...
        {
          helpText:
            "','-separated list of core modules that " +
      ...
      [
        ...
        {
          helpText:
            "','-separated list of C++ core debug " +
      ...
      [
        ...
        {
          helpText:
            `'${require('path').delimiter}'-separated list ` +
    ...
    hasIntl
      ? [
          [
            ...
            {
              helpText:
                'data path for ICU (Intl object) data' + hasSmallICU
                  ? ''
  ...
  hasNodeOptions
    ? [
        [
          ...
          {
            helpText:
              'set CLI options in the environment via a ' +
  ...
  hasCrypto
    ? [
        ...
        [
          ...
          {
            helpText:
              "sets OpenSSL's directory of trusted " +
        ...
        [
          ...
          {
            helpText:
              "sets OpenSSL's trusted certificate file " +
...
function format({
  ...
  for (const {
    ...
    for (const { 0: from, 1: to } of aliases) {
      ...
      const targetArgDescription = targetInfo
        ? getArgDescription(targetInfo.type)
        : '...'
      if (from === `${name}=`) {
    ...
    else text += StringPrototypeRepeat(' ', firstColumn - displayName.length)
